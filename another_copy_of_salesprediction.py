...
results = {'Model': [], 'CV R2 Mean': [], 'CV MAE Mean': [], 'CV RMSE Mean': []}

for name, model in models:
    # Cross-validation scores
    cv_results_r2 = cross_val_score(model, X_train, y_train, scoring='r2', cv=5)
    cv_results_mae = cross_val_score(model, X_train, y_train, scoring='neg_mean_absolute_error', cv=5)
    cv_results_rmse = np.sqrt(-cross_val_score(model, X_train, y_train, scoring='neg_mean_squared_error', cv=5))

    # Collect cross-validation results
    results['Model'].append(name)
    results['CV R2 Mean'].append("{:.3f}".format(cv_results_r2.mean()))
    results['CV MAE Mean'].append("{:.3f}".format(-cv_results_mae.mean()))  # Note the negation to get MAE
    results['CV RMSE Mean'].append("{:.3f}".format(cv_results_rmse.mean()))
...
Based on these metrics, the Lasso model consistently performs well across all metrics, followed closely by Ridge and Linear Regression (LR). RandomForest and XGBoost (XGB) also show strong performance, especially in terms of MAE and RMSE. These rankings can help you identify which models might be most suitable for your specific prediction task based on the evaluation criteria you prioritize.
...
"""Ensemble Methods: Since Random Forest and XGBoost show good performance, consider ensemble methods like stacking or boosting to further enhance predictive accuracy.
